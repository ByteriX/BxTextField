...
void printMethods(NSObject *instanceWithMethods) {
    int i=0;
    unsigned int mc = 0;
    Method * mlist = class_copyMethodList(object_getClass(instanceWithMethods), &mc);
    NSLog(@"%d methods", mc);
    for(i=0;i<mc;i++) {
        NSLog(@"Method no #%d: %s", i, sel_getName(method_getName(mlist[i])));
//        char ret[ 256 ];
//        method_getReturnType(mlist[i], ret, 256);
//        NSLog(@"Method no #%d return type: %s", i, ret);
    }
}

void savePlist(NSDictionary *dictionary, NSString *savePath) {
    NSError *writeError = nil;
    BOOL written = [dictionary writeToURL:[NSURL fileURLWithPath:savePath] error:&writeError];
    if (written) {
        NSLog(@"Successfully written to file.");
    } else {
        NSLog(@"Error: Failed write to file. %@.", [writeError localizedDescription]);
    }
}
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        if (arguments.count < 3) {
            TUPrint(@"Usage: %@ [%@] [%@]\n", arguments.firstObject.lastPathComponent, @"trace document", @"path where to save results");
            return 1;
        }
        ...
        // Check if we know where to store result
        NSString *resultPath = NSProcessInfo.processInfo.arguments[2];
        ...
        for (XRInstrument *instrument in trace.allInstrumentsList.allInstruments) {
            ...
            for (XRRun *run in runs) {
                ...
                if ([instrumentID isEqualToString:@"com.apple.xray.instrument-type.coresampler2"]) {
                    ...
                    NSMutableArray *parsedLeaks = [NSMutableArray array];
                    ...
                    for (XRLeak *leak in leaksRun.allLeaks) {
                        ...
                        [parsedLeaks addObject:parsedLeak];
                    }

                    [result setObject:parsedLeaks forKey:@"com.apple.xray.instrument-type.homeleaks"];
        ...
        // Save results in plist
        savePlist(result, resultPath);
    }
    return 0;
}
